...
function ChatInput() {
    ...
    const handleSendMessage = async () => {
        ...
        // Create new conversation if none selected (new chat clicked)
        if (!conversation) {
        ...
        // Handle error case
        if (!data || data.error) {
          const errorMessage = data?.message || "Sorry, I encountered an error processing your request. Please try again."
          dispatch(addMessage({ 
            role: "assistant", 
        ...
        console.log("Artifacts received:", data.artifacts)
    ...
    return (
        <div className='w-full oveflow-hidden px-3 md:px-5 py-4 border-t border-white/[0.06] bg-[#0d0f14'>
            <div className="flex flex-col gap-2 bg-white/[0.03] border border-white/[0.07] rounded-2xl px-4 pt-3.5 pb-3">
                <div className="flex w-[90%] gap-3 pr-2 flex-wrap">
                    {agents.map((agent) => {
